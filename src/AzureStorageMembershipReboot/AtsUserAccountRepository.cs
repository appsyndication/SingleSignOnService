...
namespace FireGiant.MembershipReboot.AzureStorage
{
    public class AtsUserAccountRepository : IUserAccountRepository<AtsUserAccount>
    {
        ...
        public AtsUserAccountRepository(AtsUserAccountConfig config, bool createIfNotExists = true, string tableName = null)
            : this(CloudStorageAccount.Parse(config.TableStorageConnectionString), config, createIfNotExists, tableName)
        {
        }

        public AtsUserAccountRepository(CloudStorageAccount storage, AtsUserAccountConfig config, bool createIfNotExists = true, string tableName = null)
        {
            _defaultTenant = config.DefaultTenant ?? "default";

            var client = storage.CreateCloudTableClient();
            _table = client.GetTableReference(tableName ?? "user");
